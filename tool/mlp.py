...
\t\t" + ("FixedNum" if useFxp else "float") + " value = m_weights[i][0];\n\
\t\tfor (int j = 0; j < m_connections[i][0]; j++){\n\
\t\t\tvalue " +\
("= fxp_sum(value, fxp_mul(m_weights[i][j+1],\
 m_value[m_connections[i][j+1]]));" if useFxp \
 else "+= m_weights[i][j+1] * m_value[m_connections[i][j+1]];") + "\n\
\t\t}\n\
\t\tif (sigmoids[i]){\n"
...
### m_outputs array
decls += "\nconst int m_outputs[OUTPUT_SIZE] = {"\
...
for i in range(len(weights)):
    ...
    decls += "\nconst " + ("FixedNum" if useFxp else "float") + " tmp_weights"\
             + str(i)\
             + "["\
             + str(len(wei))\
             + "] = {"\
             + str([toFxp(ii) if useFxp else\
                    str(ii) for ii in wei]).replace(']','').\
                    replace('[','').replace('\'','')\
             + "};\n"
    ...
    decls += "const int tmp_connections"\
             + str(i)\
             + "["\
             + str(len(con))\
             + "] = {"\
             + str(con).replace(']','').replace('[','')\
             + "};\n"
